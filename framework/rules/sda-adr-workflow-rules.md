---
description: Paths canônicos e critérios de criação do workflow ADR (Architecture Decision Records). Carregada quando trabalhando com docs/adr, skills adr-* ou skills consumidoras (orquestradores de execução e geradores que detectam/registram ADRs — usam adr.index_file e os critérios canônicos).
paths:
  - "docs/adr/**"
  - ".claude/skills/sda-adr-*/**"
  - ".claude/skills/sda-*-run*/**"
  - ".claude/skills/sda-sdd-generate-tech-spec/**"
  - ".claude/skills/sda-minispec-generate-scope/**"
...
> Paths **globais** + recursos canônicos. A skill `sda-adr-reindex` é a **dona** do script
> canônico; a skill `sda-adr-create` é a **dona** do template canônico. Demais skills do
> domínio ADR (`sda-adr-bootstrap`, `sda-adr-deprecate`, `sda-adr-supersede`) referenciam esses
...
- **adr.reindex_script**: `/.claude/skills/sda-adr-reindex/scripts/reindex.cjs`
- **adr.template**: `/.claude/skills/sda-adr-create/assets/adr-template.md`

---

# ADR — Rastreabilidade ADR ↔ Feature (convenção canônica)

> Fonte única dos nomes de seção usados na rastreabilidade bidirecional. Skills que **escrevem** ou **auditam** referências usam exatamente estes nomes.

- **Feature → ADR**: artefatos de feature (tech_spec/scope/taskcard) registram as ADRs consultadas numa subseção chamada **"ADRs Aplicáveis nesta Feature"** (dentro da seção de Observações Técnicas/Observações), uma linha por ADR no formato `ADR-NNNN — descrição curta da decisão`. Quem escreve: `sda-sdd-generate-tech-spec`, `sda-minispec-generate-scope`, `sda-taskcard-generate` (e `sda-challenge-spec` ao atualizar o artefato).
- **ADR → Feature**: a seção `## Applied in` de cada ADR (formato `feature (vN) — path-para-artefato`) é de **manutenção manual best-effort** — nenhuma skill a atualiza automaticamente. A `/sda-adr-review` (Validação 4) reporta divergências entre os dois lados como WARN para o usuário reconciliar.

---

# ADR — Critérios Canônicos de Criação (Fonte Única)

> Esta seção é a **fonte única de verdade** sobre quando criar uma ADR. Skills de detecção (`sda-sdd-generate-tech-spec`, `sda-minispec-generate-scope`) e a skill de criação (`sda-adr-create`) **referenciam** estes critérios em vez de duplicá-los. Toda nova ADR DEVE satisfazer **TODOS os 5 critérios**.
...
### Em skills de detecção (`sda-sdd-generate-tech-spec`, `sda-minispec-generate-scope`)
...
5. **NUNCA** crie ADR automaticamente — apenas sinalize. O usuário invoca `/sda-adr-create` se desejar.

### Em `sda-adr-create` (gate de criação)

A skill `sda-adr-create` deve **revalidar os 5 critérios** com o usuário (via `AskUserQuestion`) antes de coletar dados. Se qualquer critério falhar, encerra orientando o usuário a documentar a decisão no Tech Spec / Scope apropriado.
