---
description: Regras comuns do framework sda — Critical Paths (heurística de áreas sensíveis), paths compartilhados entre workflows (run_report relatório humano, workflow_report telemetria, temp_memory, pre_refinement, tech_alignment) e convenções de nomenclatura.
paths:
  - "docs/specs/**"
  - "docs/prds/**"
  - "docs/adr/**"
  ...
  - ".claude/skills/sda-mine-rule-candidates/**"
---

# sda — Regras Comuns dos Workflows

> Carregada automaticamente quando o Claude está operando qualquer workflow do framework sda (SDD, miniSpec, TaskCard, ADR) ou skills compartilhadas (sda-pre-refinement, tech-alignment).
> Centraliza Critical Paths, paths compartilhados e convenções. Paths específicos de cada workflow estão em arquivos separados (`sda-sdd-workflow-rules.md`, `sda-minispec-workflow-rules.md`, `sda-taskcard-workflow-rules.md`, `sda-adr-workflow-rules.md`).
...
### Handoff Backend → Frontend (gerado por `sda-backend-contract-handoff`)
...
- **Quem escreve**: duas skills user-invocable, com papéis distintos. **`sda-generate-design`** é a dona do **FEATURE** (`design.md`) e faz updates **cirúrgicos** no global (promoção confirmada de tokens/componentes ao longo das features — crescimento lazy). **`sda-design-system-bootstrap`** é a dona da **consolidação standalone do GLOBAL** (`design-system.md`): estrutura definições espalhadas (codebase, Figma, docs do time) sem precisar de feature como gatilho — bootstrap inicial ou enriquecimento. Skills consumidoras (tech-spec, scope, task-plan) **leem e referenciam** — nunca escrevem.
- **Quem consome**: `sda-sdd-generate-tech-spec` (seções de Fluxos de Interface e Comportamento Visual viram referência ao design em vez de redefinição), `sda-minispec-generate-scope`, geradores de tasks (design entra como arquivo de referência nas tasks de camada UI) e `sda-qa-validator` (Camada 4 — completude dos estados visuais contra o design declarado).
...
Lazy — só quando alguma skill de spec (PRD, Intent, Tech Spec, Scope) ou de challenge (`/sda-challenge-spec`) identificar terminologia que merece registro canônico. Features triviais ou puramente técnicas podem nunca ter glossário-feature, e projetos pequenos podem rodar muito tempo sem glossário-global.
...
- Skill `/sda-challenge-spec`: **dona** da criação/atualização. Durante o stress-test, ao canonizar um termo, decide com o usuário se ele vai pro **global** (cross-feature) ou **feature** (local) seguindo o critério acima.
...
> Anotados pela política débito-controlado (severidade baixa não bloqueia). Resolva tudo de uma vez com `/sda-debt-resolution docs/specs/features/{feature}/{version}/`.
...
- **Fonte primária de débito**: a Seção 2 é o que a `/sda-debt-resolution` consome — mantenha-a parseável (ver `debt-collection.md`).
...
> **O que é**: persistência **integral** do JSON retornado pelo `sda-qa-test-generator`. Quem escreve é o **orquestrador** que invocou o agente (`sda-sdd-generate-tech-spec`, `sda-sdd-generate-task-plan`, `sda-minispec-generate-tasks`, `sda-taskcard-generate`) — o contrato do agente NÃO muda (continua retornando apenas JSON). Cada caso ganha um campo extra `task_id` (`null` na fase de tech_spec; preenchido na distribuição/geração de tasks). **Semântica de escrita por contexto**: invocações da **mesma fase de geração** (fallback, batch, consolidação por camada, atualização de `task_id` na distribuição) fazem **merge append** por (`task_id`, `id`) — nunca removem casos já gravados; **re-execução da fase geradora** (ex.: regenerar o tech_spec) é regeneração — **sobrescreve** o arquivo, invalidando distribuições anteriores (o task-plan redistribui na próxima rodada).
>
> **Fluxo forward-only (canonicidade)**: generator → `_run/test-cases.json` → destrinchamento na task (subseção "Detalhamento dos Casos de Teste"). Após o destrinchamento, **a task markdown é canônica** — edições humanas acontecem nela e nunca são sobrescritas pelo JSON. Os gates (`sda-qa-validator`, `sda-staff-architecture-review`) **leem a task, nunca este arquivo**. Consumidores legítimos do JSON: redistribuição heurística do SDD (task-plan lê CTs do tech_spec sem re-parse lossy de markdown), recuperação de CTs cortados por débito (`/sda-debt-resolution`), auditoria e re-render explícito (que avisa que sobrescreve edições).
>
...
> **Para que serve**: log append-only de **sinais** que podem virar regra de projeto. Cada agente do framework (executores via os orquestradores de execução — `*-run-tasks` e `sda-taskcard-run` —, `sda-qa-validator`, `sda-staff-architecture-review`) emite linhas conforme detecta sinais canônicos durante o run. **Nenhum agente decide se vira regra** — a skill `sda-mine-rule-candidates` consolida sinais de múltiplos runs e entrega clusters para `sda-curate-project-rules` aplicar teste de fricção e definir colocação.
>
...
|---|---|---|
| `executor_askquestion` | `*-run-tasks` / `sda-taskcard-run` | Executor disparou `AskUserQuestion` (convenção ausente forçou pergunta). |
| `pre_refinement_decision` | `*-run-tasks` / `sda-taskcard-run` | Decisão registrada na subseção "Decisões já tomadas (fora de negociação)" do sda-pre-refinement (seção 11). |
| `exemplar_file_read` | `*-run-tasks` / `sda-taskcard-run` | Executor leu arquivo "exemplar" para imitar estilo (convenção não escrita). |
| `repeated_fixture` | `sda-qa-validator` | Mesma fixture/mock/setup usado em ≥2 testes do run. |
| `repeated_assertion_shape` | `sda-qa-validator` | Padrão de assert idêntico em ≥3 lugares. |
| `convention_drift` | `sda-staff-architecture-review` | Finding categoria `project_pattern` cuja causa-raiz é convenção não escrita — o Tech Review mapeia para o sinal `convention_drift`. |
| `scope_deviation` | `sda-staff-architecture-review` | Finding categoria `scope_deviation`. |
| `speculative_complexity` | `sda-staff-architecture-review` | Finding categoria `speculative_complexity`. |
...
> **Por que NÃO é tabela**: uma tabela `timestamp | signal | evidence` é compacta mas ilegível — não diz **que regra** o sinal sugere nem **o que ela faria**. O formato abaixo é **um tópico (`##`) por sinal emitido**, com a regra sugerida e uma explicação em linguagem simples, para que qualquer pessoa (ou a `sda-mine-rule-candidates`) entenda o candidato sem decifrar a evidência crua.
...
> Consumido por `/sda-mine-rule-candidates` (agrupa por tema) e `/sda-curate-project-rules` (decide se vira regra).
...
- Sinal: `executor_askquestion` · Origem: `sda-sdd-run-tasks` · 2026-05-29T14:30:00Z
...
Os agentes `sda-qa-validator` e `sda-staff-architecture-review` retornam sinais via campo `rule_candidates_emitidos[]` no JSON (não escrevem em arquivo). O orquestrador (`sda-sdd-run-tasks`, `sda-minispec-run-tasks`, `sda-taskcard-run`) é responsável por **traduzir esses sinais em linhas append-only** no `shared.rule_candidates.path`. Além disso, o próprio orquestrador emite 3 sinais que só ele observa.
...
|---|---|---|
| **Após QA aprovar/rejeitar** (`sda-qa-validator`) | Ler `rule_candidates_emitidos[]` do JSON e **anexar uma seção por item**, com `source: "sda-qa-validator"`. | `repeated_fixture`, `repeated_assertion_shape` |
| **Após Tech Review aprovar/parcial/rejeitar** (`sda-staff-architecture-review`) | Mesmo procedimento, com `source: "staff-review"`. | `convention_drift`, `scope_deviation`, `speculative_complexity` |
...
| **Fase 0 do orquestrador, ao carregar sda-pre-refinement** | Se a subseção "Decisões já tomadas (fora de negociação)" (seção 11) tem itens, append **uma seção por decisão** com `signal: "pre_refinement_decision"`, `evidence: <decisão literal>`; `tema`/`regra_sugerida`/`explicacao` derivados da decisão. | `pre_refinement_decision` |
...
> Usada por `sda-sdd-run-tasks`, `sda-minispec-run-tasks` e `sda-taskcard-run` para detectar áreas sensíveis e escalar modelo (executor e gates). **Agnóstica de linguagem/stack** — categorização por **semântica do path**, não por layout específico (Go `internal/`, Java `src/main/`, JS `src/`, Python `app/`, Dart `lib/`).
...
**Log obrigatório**: o orquestrador de execução (`*-run-tasks` e `sda-taskcard-run`) DEVE logar a fonte de `gates` antes de invocar:
...
O `sda-staff-architecture-review` retorna **`problems[]`** (lista única) onde cada item tem `severity` (`CRITICO|ALTO|MEDIO|BAIXO`) e `category`. Vocabulário canônico do TR e classificação:
...
### Categorias do `sda-qa-validator` (Gate 1)
...
> Usada por `sda-sdd-run-tasks` e `sda-minispec-run-tasks` quando o `task_plan.md` marca tasks com `Pode Rodar em Paralelo? = Sim` na mesma fase. **NÃO se aplica a `sda-taskcard-run`** — TaskCard é por definição 1 task por vez.
>
...
5. **Gates em paralelo POR TASK**: cada `ti` tem seu próprio pipeline `Agent(sda-qa-validator)` → `Agent(sda-staff-architecture-review)` que pode rodar em paralelo com os pipelines de outras tasks do lote.
...
| Versão (`{version}`) | `v1`, `v2`, ... (incremental). Variante de limpeza: `v{N+1}-debits` (criada por `sda-debt-resolution` para resolver débito anotado — skills que parseiam `{version}` devem aceitar o sufixo `-debits`) | `v1`, `v2-debits` |
