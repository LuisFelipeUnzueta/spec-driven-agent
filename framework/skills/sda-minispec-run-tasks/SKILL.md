...
description: Executa as tasks geradas pelo TASK PLAN do framework miniSpec. Coordenador de subagentes — orquestra, NÃO implementa diretamente. Para CADA task: delega ao executor (agent_name da stack), valida no Gate 1 (sda-qa-validator) e Gate 2 (sda-staff-architecture-review), aplica memória lazy em rejeições e débito-controlado (críticos/altos/médios bloqueiam; só baixos são anotados). User-invocable via /sda-minispec-run-tasks.
user-invocable: true
...
A partir de `task_plan_path`, derive `{feature}` e `{version}` para resolver os paths definidos em `.claude/rules/sda-minispec-workflow-rules.md` (paths miniSpec) e `.claude/rules/sda-workflow-rules.md` (paths compartilhados, Critical Paths e convenções).
...
## Paths (resolvidos via `.claude/rules/sda-minispec-workflow-rules.md` e `.claude/rules/sda-workflow-rules.md` — system-prompt)

Use **exclusivamente** os templates de `.claude/rules/sda-minispec-workflow-rules.md` (paths miniSpec) e `.claude/rules/sda-workflow-rules.md` (paths compartilhados), substituindo `{feature}`, `{version}`, `{n}` e `{task_id}` antes de qualquer leitura/escrita. **NUNCA** use paths hardcoded.

| Uso | Variável (sda-minispec-workflow-rules.md / sda-workflow-rules.md) |
|---|---|
...
| [`references/qa-validator-prompt.md`](references/qa-validator-prompt.md) | Prompt completo do Gate 1 (`sda-qa-validator`) + passos de preparação (3.1, 3.2, 3.3) + interpretação (3.4) + loop de correção QA (3.5). | Antes de entrar na FASE 3 (Gate 1 — QA). |
| [`references/staff-review-prompt.md`](references/staff-review-prompt.md) | Prompt completo do Gate 2 (`sda-staff-architecture-review`) + passos de preparação (4.1) + interpretação (4.3) + loop de correção (4.4) + stage `git add` (4.5) + escalação ao usuário (4.6). | Antes de entrar na FASE 4 (Gate 2 — Tech Review). |
...
| [`references/executor-discipline.md`](references/executor-discipline.md) | Bloco "Disciplina do Executor (Iron Rules)" — 7 Iron Rules anti-vícios-de-LLM (adaptação Karpathy + doutrina de testes/seam + conformidade com ADRs; "Iron Laws" é o nome reservado às 6 leis da doutrina `sda-testing-best-practices`) entre marcadores `<<<EXECUTOR_DISCIPLINE … EXECUTOR_DISCIPLINE>>>`. Arquivo canônico (symlinks em `sda-sdd-run-tasks` e `sda-taskcard-run`). | FASE 0 (carregar bloco em memória) + FASE 2.3 (injetar verbatim no TOPO do prompt de cada executor). |
...
4.2. **Instrumentação de rule mining (não-bloqueante)** — durante o run, persista candidatos a regra em `shared.rule_candidates.path` conforme a subseção **"Persistência pelo orquestrador"** de [`sda-workflow-rules.md`](.claude/rules/sda-workflow-rules.md) → "Candidatos a Regra". Trigger points no fluxo miniSpec:
   ...
   - **Pós-QA**: ao receber JSON do `sda-qa-validator`, leia `rule_candidates_emitidos[]` e anexe linha por item com `source: "sda-qa-validator"`. Dedupe intra-run.
   - **Pós-Tech Review**: idem para `sda-staff-architecture-review`, com `source: "staff-review"`.
...
7. Atualize `_run/minispec_state.yaml` (path via `minispec.state.path`):
   ...
   steps:
     execution:
       ...
       tasks_total: <N>
   ```
   Se o arquivo NÃO existir, **NÃO crie** — `sda-minispec-generate-intent` é responsável por isso.
...
## FASE 3 — Gate 1 — QA (sda-qa-validator)
...
**Resumo do fluxo**: executor → persiste `base_sha` + sumário em memória (2.4) → leitura do `references/qa-validator-prompt.md` → `Agent(sda-qa-validator, ...)` com `base_sha`/sumário INLINE em `instrucoes` → interpretar JSON → se rejeitado, montar memória lazy + prompt de correção e voltar ao executor; se aprovado, avançar para Gate 2.
...
## FASE 4 — Gate 2 — Tech Review (sda-staff-architecture-review)
...
**Resumo do fluxo**: QA aprovou → leitura do `references/staff-review-prompt.md` → preparar contexto + sumário mínimo do QA → `Agent(sda-staff-architecture-review, ...)` → interpretar JSON → se aprovado, `git add` real + marcar concluída; se reprovado, atualizar memória lazy + prompt de correção e voltar ao executor (revalidando AMBOS os gates após).
...
- **Entre tasks de um lote paralelo**, pipelines de gates rodam em paralelo (cada pipeline interno é QA→TR sequencial). Ver "Execução Paralela de Tasks" em `sda-workflow-rules.md`.
