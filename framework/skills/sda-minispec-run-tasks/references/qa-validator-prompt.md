# Prompt do Gate 1 — sda-qa-validator (FASE 3.3)

> **⚠️ ANTIDRIFT — seção espelhada entre frameworks**: o conteúdo de gates/loops deste arquivo é ESPELHO do equivalente em `sda-sdd-run-tasks/SKILL.md` (Gate 1/Passos 1-5) e `sda-taskcard-run/SKILL.md` (Passo 4 + Passo 6) — difere apenas em paths e numeração de seções. **Toda alteração DEVE ser replicada nos espelhos no mesmo PR.** Histórico: a divergência entre os 3 frameworks já produziu políticas contraditórias (zero-débito vs débito-controlado) — auditoria de jun/2026.

> Referência consumida por `SKILL.md` da skill `sda-minispec-run-tasks`.
> Leia este arquivo **antes de invocar o Gate 1** (FASE 3.3).
> Contém o prompt completo do `sda-qa-validator` e os passos de preparação (3.1, 3.2, 3.3, 3.4, 3.5).
> Use exatamente o texto da seção "Disparar o QA" como `prompt` no `Agent({...})`.
>
> **Pré-requisito de leitura**: [`config.md`](config.md) — necessário para resolver `qa_model` (regras de escalação `sda-qa-validator`) e os campos de `qa_summary_fields`. Leia `config.md` **antes** deste arquivo se ainda não o fez.
...
| `APROVADO_COM_OBSERVACOES` | 0 | ≥ 1 | QA aprovado com débito anotado → avançar para Gate 2; **acumular os baixos** para a **§2 do snapshot `_run/run-report.md`** (um bloco `### D{n} · {severidade} · {categoria} · T[N] · QA` com `Onde`=`[arquivo]:[linha]`, `Problema`=`titulo`, `Impacto`=`descricao`, `O que fazer`=`correcao_sugerida` — formato em `sda-workflow-rules.md`) |
| `REJEITADO` | ≥ 1 | qualquer | Enviar críticos+altos+médios como bloqueantes ao executor (3.5); baixos como observações opcionais |

> **Sinal `stack_discovery.discovery_needed: true`** (não afeta veredito): o QA não resolveu um detalhe **não-derivável do código** (ex.: framework E2E não padronizado, política de cobertura). Recomende ao usuário rodar **`/sda-testing-stack-bootstrap`** — ele descobre a stack do código, pergunta só o não-derivável e gera `.claude/rules/testing-stack.md`. A partir daí o QA resolve a stack automaticamente. Não bloqueie o pipeline por esse sinal.
...
**Ao fechar o loop com aprovação**: acumule para a §2 do snapshot `_run/run-report.md` os baixos remanescentes do último JSON do QA que NÃO foram corrigidos (um bloco `### D{n} · {severidade} · {categoria} · T[N] · QA` por problema, com `Onde`=`[arquivo]:[linha]`, `Problema`=`titulo`, `Impacto`=`descricao`, `O que fazer`=`correcao_sugerida`; `arquivo`/`linha`/`correcao_sugerida` vêm do próprio problema no JSON — não os descarte: alimentam as tasks de cleanup da `/sda-debt-resolution`) — o caminho "REJEITADO → corrigido → aprovado" não passa pelo registro automático do veredito `APROVADO_COM_OBSERVACOES`. (Médios já não chegam aqui como débito: foram corrigidos no loop.)
