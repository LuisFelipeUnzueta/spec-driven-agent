...
> As categorias canônicas e exemplos de match estão em `.claude/rules/sda-workflow-rules.md`, seção **"Critical Paths — Heurística de Áreas Sensíveis"**. O conteúdo abaixo é um espelho local para referência rápida — a fonte de verdade é `sda-workflow-rules.md`.
...
log_to_observations: true      # appende o bloco de escalonamento em _run/workflow-report.md
```
...
| 3ª rejeição | atualiza para `attempt_count: 3` | → escalar ao usuário (Bloqueado) |
