...
description: Gera uma TaskCard individual clara e executável para uma task específica do framework TaskCard. Conduz o usuário através de processo interativo (uma pergunta por vez), preenche o template oficial (seções 1-9 e 11), salva em disco, delega a seção 10 (Testes) ao agente sda-qa-test-generator e formata o resultado. User-invocable via /sda-taskcard-generate.
user-invocable: true
disable-model-invocation: true
argument-hint: [--mode=crud-fastpath] <contexto da tarefa ou Intent + Scope>
---

# Skill: sda-taskcard-generate
...
Você domina completamente o framework: template, regras, guardrails, convenções de nomenclatura, estrutura de diretórios e o fluxo de geração com delegação ao `sda-qa-test-generator`.
...
Variáveis usadas nesta skill: `pre_refinement.path`, `taskcard.task_plan.path`, `taskcard.tasks.dir`, `taskcard.tasks.pattern`. Templates definidos em `.claude/rules/sda-taskcard-workflow-rules.md` (paths TaskCard) e `.claude/rules/sda-workflow-rules.md` (paths compartilhados).
...
**PRIMEIRA AÇÃO da skill, antes de detectar modo ou ler o sda-pre-refinement.** TaskCard usa **template único**, mas a frente escolhida é necessária para:

- Alimentar o subagente `sda-qa-test-generator` (parâmetro `frente`) na FASE 6 — orienta a escolha de stacks de teste (Playwright/Cypress p/ Web, Patrol/Detox/Appium p/ Mobile, Go test/pytest/etc. p/ Backend).
...
2. **Pergunta obrigatória** (sempre via `AskUserQuestion`):
   > "Qual é a frente desta TaskCard? Define o `frente` enviado ao sda-qa-test-generator e o campo `Variante` da TaskCard."
   > Opções: `Web` | `Mobile` | `Backend`

3. **Persistência** — a frente escolhida (`web`, `mobile` ou `backend`) deve ser usada:
   - Na FASE 6 ao invocar o `sda-qa-test-generator` (parâmetro `frente`).
...
> **Por que SEMPRE perguntar**: TaskCard frequentemente é invocada sem upstream (`scope.md`/`tech_spec.md`) — não há de onde inferir a frente. Mesmo quando há upstream, o tech-alignment é opcional e nem sempre confiável. Pergunta explícita custa 1 turn e elimina ambiguidade — espelha a regra de `sda-minispec-generate-scope` (FASE 0.0) e `sda-sdd-generate-tech-spec` (FASE 0).
...
⚠️  O pre-refinement.md desta feature recomenda rodar em <FRAMEWORK>,
    mas você invocou /sda-taskcard-generate.
...
**Lógica completa**: ver `.claude/rules/sda-workflow-rules.md` (seções "Critical Paths — Heurística de Áreas Sensíveis", "Executor model rules" e "Gates inference rules") — já carregada no system-prompt.
...
4. **Se EXISTIR** → liste versões existentes (v1, v2, ...), identifique a mais recente (vN) e pergunte ao usuário usando `AskUserQuestion`. **Sufixo `-debits`**: ignore versões `v{N}-debits` (limpeza gerada pela `/sda-debt-resolution`) ao calcular o vN mais recente e o vN+1 — elas não contam como versão funcional; liste-as separadamente na pergunta e **nunca** as sobrescreva.
...
> **NUNCA** use paths hardcoded como `docs/<feature>/vN/...`. Sempre resolva via `taskcard.*` definidos em `.claude/rules/sda-taskcard-workflow-rules.md`.
...
## FASE 6 — Delegação ao sda-qa-test-generator (SEÇÃO 10)
...
>
> **Importante**: a seção 10 deve ser gerada pelo sda-qa-test-generator **independentemente** do valor de `gates`. O valor de `gates` afeta apenas a **execução** (sda-taskcard-run) — não a geração dos testes aqui.
>
> **Única exceção (N/A)**: se `gates: none` foi inferido por `tipo` = `docs` / `config_isolada` / `constantes_isoladas` (task sem código executável de domínio), **NÃO dispare o agente** — preencha a seção 10 com `N/A — task não envolve código testável` (literal que o sda-taskcard-run reconhece para omitir a instrução de doutrina ao executor). Disparar o test-generator para docs/config gera CTs de baixo valor a custo de subagente.

### Modo Batch — múltiplas TaskCards em sequência

Se o usuário estiver gerando **múltiplas TaskCards** em sequência (ex.: 5 TaskCards relacionadas), o custo fixo de 1 subagente sda-qa-test-generator por TaskCard (~9-12k de system prompt + MCP) é multiplicado. Para 5 TaskCards = 45-60k de overhead.
...
2. Para cada domínio com ≥ 2 TaskCards, dispare **1 subagente sda-qa-test-generator** passando TODAS as TaskCards do domínio em `arquivos[]`.
3. Em `instruções`, ative o **modo batch do contrato do agente** (seção "MODO BATCH" de `sda-qa-test-generator.md`): o retorno é um envelope JSON com chave por TaskCard ID — `{"TC-001": <schema canônico>, "TC-002": <schema canônico>}` — onde cada valor segue o schema completo do agente (`casos_teste[]`, `stack_discovery`, etc.). A FASE 7 formata a §10 de cada card a partir da sua chave.
...
### Disparar o sda-qa-test-generator

Lance usando a ferramenta `Agent` com:
- **subagent_type**: `sda-qa-test-generator`
...
### 6.5.3 1 chamada batched do sda-qa-test-generator (FASE 6 substituída)

Em vez de 1 chamada por TaskCard, dispare **1 chamada única do `sda-qa-test-generator`** cobrindo a feature inteira (é o contrato normal do agente — a "feature inteira" vai como instrução, não como modo):
...
O `sda-qa-test-generator` retorna um JSON estruturado. Você DEVE transformar esse JSON na seção 10 do template oficial e editar o arquivo da TaskCard. O formato de destino é:

```markdown
## 10. Testes

> Gerado pelo agente `sda-qa-test-generator` em [data].
...
2. **Tabela de execução** com ordem e dependências (SEM coluna de paralelismo — o `sda-taskcard-run` executa estritamente 1 card por invocação, na ordem que o usuário escolher; a regra de Execução Paralela da rule compartilhada exclui o TaskCard nominalmente):
...
> A coluna `Status` é atualizada pelo `sda-taskcard-run` (Em Progresso → Concluído/Bloqueado) — mesma transição da seção 1 de cada card.
...
- Arquivo: path resolvido a partir de `taskcard.task_plan.path` em `.claude/rules/sda-taskcard-workflow-rules.md` (com `{feature}` e `{version}` substituídos)
...
5. **Template completo** — seções 1-9 e 11 preenchidas pelo gerador; seção 10 (Testes) **OBRIGATORIAMENTE delegada** ao agente `sda-qa-test-generator`.
...
9. **NUNCA inicie automaticamente a próxima etapa** (`sda-taskcard-run`) — apenas encerre e aguarde.
10. **NUNCA use paths hardcoded** — sempre resolva via `taskcard.*` definidos em `.claude/rules/sda-taskcard-workflow-rules.md`.
...
> Convenções gerais (`{feature}`, `{version}`) em `sda-workflow-rules.md`.
...
Contexto do usuário
  ...
  -> FASE 6 / 6.5: Disparar sda-qa-test-generator
...
- [ ] `sda-qa-test-generator` disparado e seção 10 formatada (subseções 10.1 a 10.6)
