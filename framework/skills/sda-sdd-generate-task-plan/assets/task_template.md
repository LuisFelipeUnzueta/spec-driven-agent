...
- **Fase**:
- **Dependências**:
- **Símbolos públicos criados**:        <!-- tipos/funções/interfaces/constantes que OUTRAS tasks podem consumir (ex.: `service.EmailSender`, `dto.CartItem`). N/A se nenhum. Alimenta a derivação do flag de paralelismo e o guard de disjunção de símbolo do executor (ver sda-workflow-rules.md → "Invariante de Paralelismo"). -->
...
<!-- LLM-ONLY: Os nomes de arquivo, função e framework são SEMPRE na convenção da stack descoberta pelo
  sda-qa-test-generator (campos `stack_discovery` e `existing_suite` do JSON). Não pressuponha
