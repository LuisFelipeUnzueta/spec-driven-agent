...
**Mapeamento slug da pasta → slug da doc**: as pastas das skills usam o prefixo de distribuição `sda-` (ex.: `sda-sdd-generate-prd`); as páginas do site **não** usam o prefixo (ex.: `skills/sdd/sdd-generate-prd.md`). Derive `slug_doc = slug_pasta` **sem** o prefixo `sda-`.

**Convenção de grupo da skill** (inferida pelo slug da pasta):
- `sda-sdd-*` → `skills/sdd/`
- `sda-minispec-*` → `skills/minispec/`
- `sda-taskcard-*` → `skills/taskcard/`
- `sda-adr-*` → `skills/adr/`
- Demais (`sda-<resto>`) → `skills/shared/`

**Agents**: mesma regra de strip do prefixo (`sda-qa-validator.md` → `docs/site/docs/agents/qa-validator.md`), com uma exceção mapeada: `sda-staff-architecture-review.md` → `agents/staff-architecture-review-agent.md`.
