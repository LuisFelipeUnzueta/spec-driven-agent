...
4. **Varrer o design system real no codebase** — o que está implementado vence o que está documentado. A busca é **semântica e agnóstica de stack** (mesma filosofia dos Critical Paths em `sda-workflow-rules.md`): procure pelos **conceitos**, em qualquer linguagem — os nomes abaixo são exemplos, **não lista fechada**:
...
Antes de escrever, classifique cada decisão coletada usando a tabela da rule comum (`sda-workflow-rules.md` → "Design — Dois Níveis"):
