...
8. **Fronteira com a generate-design.** Esta skill escreve **apenas o GLOBAL**. Telas, layouts e estados visuais de uma feature são do `design.md` da feature — território da `sda-generate-design`. Se o usuário pedir design de feature, redirecione.

---

## Saída desta skill

Um único artefato: o **`design-system.md`** global no path resolvido via **`design_system.global.path`** (`/docs/specs/design-system.md` — ver `sda-workflow-rules.md`, seção "Design — Dois Níveis"). É a fonte canônica da identidade visual que `sda-generate-design`, tech-spec, scope, geradores de tasks e `sda-qa-validator` (Camada 4) consomem.
