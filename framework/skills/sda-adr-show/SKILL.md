...
Os paths vêm de `.claude/rules/sda-adr-workflow-rules.md` (rule global no system-prompt). Use sempre:
...
**NUNCA** assuma paths hardcoded fora dos definidos em `.claude/rules/sda-adr-workflow-rules.md`.
...
1. **Parse do argumento**:
   - Se for puramente numérico: `id_padded` = padding de string para 4 dígitos. **Cuidado em bash**: `printf "%04d" 0010` interpreta o operando como OCTAL (resultado `0008`) e `0009` falha — se usar `printf`, force decimal com `printf "%04d" $((10#$ARGUMENTS))`; mais simples: se já tem 4 dígitos use como está, senão prefixe zeros. Padrão de busca: `{adr.dir}/{id_padded}-*.md`.
...
2. Respeitar os paths de `.claude/rules/sda-adr-workflow-rules.md` (`adr.dir`, `adr.file_pattern`, `adr.index_file`). Se algum path estiver ausente do system-prompt, avise o usuário e encerre.
