...
A seção de testes do TECH_SPEC **NÃO** deve ser preenchida pelo arquiteto. Você DEVE delegar para o subagente **`sda-qa-test-generator`** que retorna um JSON estruturado. Depois, você converte o JSON em markdown para a seção correta.
...
1. **frente**: <web | mobile | backend>          ← NOVO. Deriva de FASE 0 da skill sda-sdd-generate-tech-spec.
...
> A deduplicação **CT→task** acontece na fase de task plan (tasks ainda não existem aqui) — a regra completa vive em `sda-sdd-generate-task-plan/references/qa-delegation-tasks.md`. Na fase de tech_spec, garanta apenas:
