...
> **Tipo**: Versão de débitos (gerada por `sda-debt-resolution`).
> **Origem**: `docs/specs/features/{{feature}}/{{parent_version}}/_run/report.md`
> **Variante**: {{variant}}
> **Data**: {{data}}
...
<!-- LLM-ONLY: Esta versão NÃO adiciona funcionalidade. É exclusivamente cleanup
     ...
     política débito-controlado dos validation durante a execução de `{{parent_version}}`.
...
- **Origem dos débitos**: `docs/specs/features/{{feature}}/{{parent_version}}/_run/report.md`
...
Resolver **{{count_selecionados}} débitos técnicos** acumulados na execução de `{{parent_version}}`, classificados como aceitáveis para passar pelos validation (severidade `MEDIO`/`BAIXO`, categorias `code_review_only`) mas que prejudicam manutenibilidade, legibilidade ou propagam anti-padrões se deixados sem ação.

A versão é gerada via skill `sda-debt-resolution` que:

1. Coletou {{count_coletado}} débitos elegíveis de `_run/report.md` da `{{parent_version}}`.
...
Após execução desta versão via `sda-minispec-run-tasks`:

- Cada débito selecionado vira **1 task atômica** em `tasks/T{n}.md` com `validation: qa`.
...
- A `{{parent_version}}` está concluída (todas as tasks principais aprovadas pelos validation).
- Os débitos coletados refletem o estado real após a última execução de `sda-minispec-run-tasks` na `{{parent_version}}`.
...
- **Refactor arquitetural**: débitos que exigiriam ADR ou mudança de padrão **não** entram aqui — seriam classificados como `revalidation_required` pelos validation e teriam bloqueado o pipeline da `{{parent_version}}`. Se algum aparece, é bug do gate.
- **Tech Review profundo**: tasks de débito têm `validation: qa` por default (categoria `code_review_only`).
...
```
sda-minispec-run-tasks docs/specs/features/{{feature}}/{{version}}/task_plan.md
```
