...
when_to_use: |
  - Depois de atualizar o framework sda para a versão com layout `_run/` +
...
do_not_invoke_for: |
  ...
  - Resolver débito técnico anotado (use sda-debt-resolution).
...
```bash
python3 .claude/skills/sda-migrate-specs/assets/migrate.py --root .
```
...
```bash
# todas as features:
python3 .claude/skills/sda-migrate-specs/assets/migrate.py --root . --apply

# ou uma feature/version específica:
python3 .claude/skills/sda-migrate-specs/assets/migrate.py --feature docs/specs/features/<feature>/<version> --apply
```
...
- **Back-compat continua valendo**: features que o usuário escolher NÃO migrar seguem funcionando (resume/debt-resolution caem no layout plano antigo — ver `sda-workflow-rules.md` → callout "Pasta `_run/`"). A migração é uma limpeza opcional, não um pré-requisito.
...
- **Companion de doc**: ao adicionar/alterar esta skill no framework, gere a página da Referência via `/sda-docs-sync`.
