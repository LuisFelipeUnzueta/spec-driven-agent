...
>
> Para cada antipattern: **nome canônico** (em inglês, igual ao JSON), **família**, **gate question** (pergunta que detecta), **fix**, **severidade sugerida** para `sda-qa-validator.testing_smells`.

---

## Família 1: Brittleness (AP-01..AP-06; a família tem 7 com o AP-29, listado na seção de adendos)
...
## Família 2: Flakiness (AP-07..AP-09)
...
## Família 3: Mock misuse (AP-10..AP-14; a família tem 6 com o AP-27, listado na seção de adendos)
...
## Família 4: Process (AP-15..AP-24; a família tem 12 com AP-26/AP-28, listados na seção de adendos)
...
## Família 5: AI-specific e adendos pós-post-mortem (AP-25..AP-29)

> AP-25 é o único da família AI-specific. AP-26..AP-29 são adendos catalogados aqui por ordem cronológica de descoberta, mas pertencem às famílias declaradas em seus corpos (AP-26/AP-28 → Process; AP-27 → Mock misuse; AP-29 → Brittleness).
...
- **Heurística determinística (sda-qa-validator usa)**: ignorar `it.each`/`test.each`/table-driven — um único teste parametrizado **não** é duplicata. Reportar apenas pares de funções de teste autonômas com tuplas equivalentes.

> Esse antipadrão apareceu no post-mortem `cadastro-pratos-franquia` T8: CT-001 e CT-002 em `franchise_dish_routes_test.go` eram semanticamente idênticos e foram aprovados pelo QA com nota 9. Tech Review apontou tarde. Por isso virou heurística determinística no Camada 5 do `sda-qa-validator`.
