...
**Heurística de detecção em revisão (QA / sda-qa-test-generator)**: se o arquivo de teste do `XRepository` declara `type fakeXRepository` ou `mockXRepository` cujos métodos têm a **mesma assinatura do próprio Repository**, é mock-driven confidence (AP-10) — peça extração da `XQuerier` interface mínima.
