...
Conteúdo fora dos marcadores (cabeçalho, instruções, linha `Ultima atualizacao`) é **preservado**. Se um dos marcadores estiver ausente, o script falha com erro claro — **não** tente recriar o `INDEX.md` daqui (é responsabilidade de `sda-adr-create` / `sda-adr-bootstrap`).
...
```
[sda-adr-reindex] INDEX.md atualizado — N ADR(s) listadas.
[sda-adr-reindex] AVISO: ... (se houver — exibir todas as linhas de aviso)
```
