...
description: Executa as tasks geradas pelo TASK PLAN do framework SDD. Coordenador de subagentes — orquestra, NÃO implementa diretamente. Para CADA task: delega ao executor (agent_name da stack), valida no Gate 1 (sda-qa-validator) e Gate 2 (sda-staff-architecture-review), aplica memória lazy em rejeições e débito-controlado (críticos/altos/médios bloqueiam; só baixos são anotados). User-invocable.
user-invocable: true
...
A partir de `task_plan_path`, derive `{feature}` e `{version}` para resolver os paths definidos em `.claude/rules/sda-sdd-workflow-rules.md` (paths SDD) e `.claude/rules/sda-workflow-rules.md` (paths compartilhados, Critical Paths e convenções).
...
## Paths (resolvidos via `.claude/rules/sda-sdd-workflow-rules.md` e `.claude/rules/sda-workflow-rules.md` — system-prompt)

Use **exclusivamente** os templates de `.claude/rules/sda-sdd-workflow-rules.md` (paths SDD) e `.claude/rules/sda-workflow-rules.md` (paths compartilhados), substituindo `{feature}`, `{version}` e `{task_id}` antes de qualquer leitura/escrita. **NUNCA** use paths hardcoded.

| Uso | Variável (sda-sdd-workflow-rules.md / sda-workflow-rules.md) |
|---|---|
...
Como aplicar:
- Cruze os arquivos declarados (seções 5.1 e 5.2 da task) com as categorias de `sda-workflow-rules.md` (case-insensitive, semântico).
...
Aplicadas APENAS se o frontmatter da task NÃO declarar `model:`. Regras canônicas (ordem de avaliação, primeira que casar vence) definidas em `.claude/rules/sda-workflow-rules.md` → seção **"Executor model rules (compartilhadas)"**.
...
4.2. **Instrumentação de rule mining (não-bloqueante)** — durante o run, persista candidatos a regra em `shared.rule_candidates.path` conforme a subseção **"Persistência pelo orquestrador"** de [`sda-workflow-rules.md`](.claude/rules/sda-workflow-rules.md) → "Candidatos a Regra". Trigger points no fluxo SDD:
   ...
   - **Passo 6 (pós-QA)**: ao receber JSON do `sda-qa-validator`, leia `rule_candidates_emitidos[]` e anexe uma linha por item com `source: "sda-qa-validator"`. Dedupe intra-run.
   - **Passo 7 (pós-Tech Review)**: idem para `sda-staff-architecture-review`, com `source: "staff-review"`.
...
5. Atualize `_run/sdd_state.yaml` (path via `sdd.state.path`):
   ...
   steps:
     execution:
       ...
       tasks_total: <N>
   ```
   Se o arquivo NÃO existir, **NÃO crie** — `sda-sdd-generate-prd` é responsável por isso.
...
9. **Gates por task em paralelo**: para cada `ti` do lote, despache `Agent(sda-qa-validator)` em paralelo numa única mensagem. Aguarde TODOS. Para os que aprovaram, despache `Agent(sda-staff-architecture-review)` em paralelo. Dentro de uma mesma task, QA → Tech Review continua sequencial.
...
> **Formato canônico do bloco de débito (§2 do `_run/run-report.md`)** — todo baixo anotado (QA `APROVADO_COM_OBSERVACOES`, baixo remanescente ao fechar o loop, Tech Review `APROVADO_COM_OBSERVACOES`) vira um bloco neste formato. **NUNCA descarte `arquivo`/`linha`/`correcao_sugerida`** — alimentam a `/sda-debt-resolution`:
>
> ```markdown
...
**Ao fechar o loop com aprovação**: acumule para a §2 do snapshot `_run/run-report.md` os baixos remanescentes do último JSON do QA que NÃO foram corrigidos — um bloco por problema no formato canônico (`### D{n} · {severidade} · {categoria} · T[N] · QA` com Onde/Problema/Impacto/O que fazer); `arquivo`/`linha`/`correcao_sugerida` vêm do próprio problema no JSON — **nunca os descarte**: alimentam as tasks de cleanup da `/sda-debt-resolution`. O caminho "REJEITADO → corrigido → aprovado" não passa pelo registro automático do veredito `APROVADO_COM_OBSERVACOES`. (Médios já não chegam aqui como débito: foram corrigidos no loop.)
...
2. **Extraia os problemas — política débito-controlado**:
   ...
   - **Acumule AGORA os baixos para a §2 do snapshot `_run/run-report.md`** — um bloco por problema no formato canônico (`### D{n} · {severity} · {category} · T[N] · Tech Review` com Onde/Problema/Impacto/O que fazer); `arquivo`/`linha`/`suggested_fix` vêm do próprio item em `problems[]` — **nunca os descarte**: são o que permite à `/sda-debt-resolution` gerar tasks de cleanup sem reinferir paths. O prompt de correção afirma que eles "já estão anotados na §2"; este é o passo que garante isso.
...
4. **Classifique `requires_qa_revalidation`** aplicando a regra "Tech Review Correction — Classificação `requires_qa_revalidation`" de `.claude/rules/sda-workflow-rules.md`:
...
> O `_run/run-report.md` é um **snapshot regenerável** (NÃO append-only) — estrutura canônica fixa de 4 seções definida em `sda-workflow-rules.md` → "Relatório do Run". Reescreva-o **por inteiro** a cada estado terminal de task e ao fim do run.
...
6. **NUNCA usar paths hardcoded** — sempre resolva via templates do `.claude/rules/sda-sdd-workflow-rules.md` (paths SDD) e `.claude/rules/sda-workflow-rules.md` (paths compartilhados).
...
- **Débitos Técnicos Não Resolvidos** (cada baixo anotado como bloco `### D{n} · sev · cat · task · gate` com Onde/Problema/Impacto/O que fazer — vira a §2 do snapshot) + ponteiro de fechamento de ciclo: "Para transformar o débito em versão de limpeza, rode `/sda-debt-resolution <feature_path>`". **NÃO auto-execute** — a decisão é do usuário.
